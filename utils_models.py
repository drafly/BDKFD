...
#####local model
class client_local_model(nn.Module):
    def __init__(self, name, args=True):
        super(client_local_model, self).__init__()
        self.name = name
        if self.name == 'Linear':
            [self.n_dim, self.n_out] = args
            self.fc = nn.Linear(self.n_dim, self.n_out)
        ...
        #     self.pool = nn.MaxPool2d(3, 2)
        #     self.fc1 = nn.Linear(256 * 11 * 11, 4096)
