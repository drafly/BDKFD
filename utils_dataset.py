    ...
    def set_data(self):
        ...
        if not os.path.exists('%sData/%s' % (self.data_path, self.name)):
            if self.dataset == 'Kvasir':
                data_dir = './data/Raw/Kvasir/'
                apply_transform = transforms.Compose(
                    [
                        transforms.Resize((227, 227)),
                        transforms.ToTensor(),
                        transforms.Normalize((0.4855145, 0.3459037, 0.29825866),
                                             (0.2991343, 0.22708875, 0.20316184))])

                trnset = torchvision.datasets.ImageFolder('%sData/Raw/Kvasir/' % self.data_path,
                                                          transform=apply_transform)

                tstset = torchvision.datasets.ImageFolder('%sData/Raw/Kvasir/' % self.data_path,
                                                          transform=apply_transform)
                print(trnset.classes)

                trn_load = torch.utils.data.DataLoader(trnset, batch_size=6000, shuffle=True, num_workers=0)
                tst_load = torch.utils.data.DataLoader(tstset, batch_size=2000, shuffle=True, num_workers=0)
                self.channels = 3;
                self.width = 227;
                self.height = 227;
                self.n_cls = 8;
            ...
            print(n_data_per_clnt)

            # Draw from lognormal distribution
            clnt_data_list = (np.random.lognormal(mean=np.log(n_data_per_clnt), sigma=self.unbalanced_sgm,
                                                  size=self.n_client))
            clnt_data_list = (clnt_data_list / np.sum(clnt_data_list) * len(trn_y)).astype(
                int)
            ...
            if self.rule == 'Drichlet':
                ...
                cls_priors = np.random.dirichlet(alpha=[self.rule_arg] * self.n_cls,
                                                 size=self.n_client)
                ...
                clnt_y = [np.zeros((clnt_data_list[clnt__], 1)).astype(np.int64) for clnt__ in
                          range(self.n_client)]
        ...
        if self.name == 'mnist' or self.name == 'synt' or self.name == 'emnist':
            ...
            if self.train:
                ...
                if (np.random.rand() > .5):
                    ...
                    dim_1, dim_2 = np.random.randint(pad * 2 + 1, size=2)#生成随机数，不包括9
                    img = extended_img[:, dim_1:dim_1 + 32, dim_2:dim_2 + 32]#将原始图片进行裁剪
            img = np.moveaxis(img, 0, -1)#将img的第一维移动到最后一维，其余顺序维度不变，和np.transpose功能一样
